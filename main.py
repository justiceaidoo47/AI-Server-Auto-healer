...
def analyze_error_with_ai(error_text):
    ...
    1. "root_cause": (brief explanation)
    2. "fix_command": (the exact terminal command to run to fix this)
    3. "risk": (say "High", "Medium", or "Low")
    """
    ...
    try:
        ...
        print(f"[!] AI Error: {e}")
        return {"root_cause": "Unknown", "fix_command": "systemctl restart nginx", "risk": "High"}

def save_report(original_error, ai_result):
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    report_data = {"timestamp": timestamp, "error": original_error, "diagnosis": ai_result}
